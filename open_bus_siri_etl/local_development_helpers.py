...
def download_latest_snapshots():
    ...
    for i in reversed(range(1, 120)):
        filename = (now - datetime.timedelta(minutes=i)).strftime('/%Y/%m/%d/%H/%M.br')
        url = REMOTE_URL + filename
        filepath = os.path.join(open_bus_siri_requester.config.OPEN_BUS_SIRI_STORAGE_ROOTPATH, filename.strip('/'))
